...
<div class="header">
  ...
  <div style="margin-top:10px;display:flex;gap:10px;flex-wrap:wrap">
    <a href="/restock/" style="background:#fff;color:#1e3a8a;text-decoration:none;padding:8px 16px;border-radius:6px;font-weight:700;font-size:13px">📦 收貨管理</a>
    <a href="/checkout/" style="background:#fff;color:#1e3a8a;text-decoration:none;padding:8px 16px;border-radius:6px;font-weight:700;font-size:13px">🛒 結帳作業</a>
  </div>
</div>
<div class="container">
...
document.addEventListener("DOMContentLoaded",function(){{  // OOS/LOS auto-fill 叫貨量
  ...
  // fill product select from API
  fetch("/api/product_options").then(function(r){{return r.json()}}).then(function(prods){{
    ...
    sel.addEventListener("change",function(){{renderSales(this.value)}});
  }});
  // load sales data and render chart
  fetch("/api/daily_sales").then(function(r){{return r.json()}}).then(function(data){{
    window.allSalesData=data;
    window.productMap={{}};
    document.querySelectorAll("#sales-product-select option").forEach(function(o){{
      if(o.value!=="__all__") window.productMap[o.value]=o.textContent;
    }});
    renderSales("__all__");
  }}).catch(function(e){{console.error("sales fetch error",e)}});
...
function renderSales(productId){{
  ...
  if(productId!=="__all__"){{
    filtered=allSalesData.filter(function(r){{return r.product_id===productId}});
  }}
